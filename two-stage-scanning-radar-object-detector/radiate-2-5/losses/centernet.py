...
from utils.tensor import _transpose_and_gather_feat, _sigmoid, _relu
import numpy as np
...
class DetectionLoss(torch.nn.Module):
    ...
    def forward(self, output, batch):
        ...
        if output.get('two_stage_traning_loss_dict'):
            loss_stats = {**loss_stats, **output['two_stage_traning_loss_dict']}
...
class RegL1Loss(torch.nn.Module):
    ...
    def forward(self, output, mask, ind, target):
        ...
        if pred.shape[-1] == 3:
            pred_wh = pred[..., 0:2]
            pred_angle = _relu(pred[..., 2:3])

            target_wh = target[..., 0:2]
            target_angle = _relu(target[..., 2:3])
