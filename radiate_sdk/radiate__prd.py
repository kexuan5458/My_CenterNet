...
class Sequence:
    ...
    def vis_all(self, output, wait_time=1, frame_count=0):
        ...
        if (output != {}):
            ...
            if self.config['use_radar_cartesian']:
                radar_cart_vis = self.vis(
                    output['sensors']['radar_cartesian'], output['annotations']['radar_cartesian'], color=None, mode='rot', frame_count=fc)
            ...
            if (self.config['use_lidar_bev_image']):
                lidar_vis = self.vis(
                    output['sensors']['lidar_bev_image'], output['annotations']['lidar_bev_image'], color=None, mode='rot', frame_count=fc)
                # def vis(self, sensor, objects, color=None, mode='rot', frame_count=0):
    ...
    def vis(self, sensor, objects, scores=[], frame_count=0, color=None, mode='rot'):
        # radar_cart_vis = self.vis(
        ...
        print("vis():" + str(frame_count))
        objs = []
        sensor_vis = np.copy(sensor)
        # frame_token = ( str(f"{frame_count:06d}") + '.png')
        frame_token = ( str(f"{frame_count:06d}"))
        for idx, object in enumerate(objects):
            bbox = object['bbox']['position']
            angle = object['bbox']['rotation']

            class_name = object['class_name']
            if (class_name != 'pedestrian' and class_name != 'group_of_pedestrians'):
        ...
        with open('prd.json', 'a') as outfile:
